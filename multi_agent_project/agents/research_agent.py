...
class ResearchAgent:
    """Research Agent for web and data analysis."""
    ...
    async def stream(self, query: str) -> AsyncGenerator[dict[str, Any], None]:
        ...
        logger.info(f"Streaming response for research query: {query}")
        try:
            messages = [
                SystemMessage(
                    content="""
                    You are an expert research assistant specializing in gathering and analyzing information 
                    from various sources. Your goal is to provide accurate, well-structured, and sourced 
                    information based on the user's query.

                    When providing information:
                    - Conduct thorough research and verify facts
